...
const TOOLS = [
  {
    ...
    description:
      'Search across all documents in the Kite workspace. Returns titles, slugs, previews, and relevance scores.',
    inputSchema: {
  ...
  },
  {
    ...
    description:
      'Publish a document (transition from draft/review to published).',
    inputSchema: {
      type: 'object' as const,
      properties: {
        slug: { type: 'string', description: 'Document slug or ID' },
  ...
  },
  {
    ...
    description:
      'List all OpenAPI specification sources in the workspace.',
    inputSchema: {
      type: 'object' as const,
      properties: {},
    },
  },
  {
    ...
    description:
      'List API endpoints from an OpenAPI source. Shows method, path, summary.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        source: {
...
async function handleToolCall(
  ...
  switch (name) {
    case 'search_documents': {
      ...
      if (!data.results?.length) return text('No results found.')
      const lines = data.results.map(
        (r) =>
          `- **${r.title}** (${r.slug || r.documentId})${r.preview ? `\n  ${r.preview.slice(0, 200)}` : ''}`,
      )
    ...
    case 'list_documents': {
      ...
      if (!data.items.length) return text('No documents found.')
      const lines = data.items.map(
        (d) => `- [${d.status}] **${d.title}** — \`${d.slug}\``,
      )
      return text(lines.join('\n'))
    }

    case 'get_document': {
      const doc = await fetchDocument(args.slug as string)
      if (!doc) return text(`Document "${args.slug}" not found.`)
      return text(
        `# ${doc.title}\n\n` +
    ...
    case 'list_openapi_sources': {
      ...
      if (!data.items.length) return text('No OpenAPI sources found.')
      const lines = data.items.map(
        (s) =>
          `- **${s.name}** (${s.openapiVersion || 'unknown'}) — ${s.sourceType}${s.lastSyncedAt ? ` — synced: ${s.lastSyncedAt}` : ''}`,
      )
      return text(lines.join('\n'))
    }
    ...
    case 'list_api_endpoints': {
      ...
      const source = sources.items.find(
        (s) => s.id === (args.source as string) || s.name === (args.source as string),
      )
      if (!source) return text(`OpenAPI source "${args.source}" not found.`)
      const data = await fetchOpenapiEndpoints(source.id)
      if (!data.endpoints?.length) return text('No endpoints found.')
      const lines = data.endpoints.map(
        (ep) =>
          `- \`${ep.method.toUpperCase()} ${ep.path}\`${ep.summary ? ` — ${ep.summary}` : ''}${ep.deprecated ? ' *(deprecated)*' : ''}`,
      )
      return text(lines.join('\n'))
    }
